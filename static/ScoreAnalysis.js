...
// ── SA body class cleanup (App_HideAllViews is an org script — patch it) ──────
document.addEventListener('DOMContentLoaded', () => {
  const _orig_HideAll = window.App_HideAllViews;
  window.App_HideAllViews = function() {
    if (_orig_HideAll) _orig_HideAll();
...
// ── Tutorial ──────────────────────────────────────────────────────────────────
const _SA_TOUR_STEPS = [
  { title: 'Score Config',   body: 'Select your <strong>fraud score column</strong>, set a <strong>Start / End</strong> range and a <strong>Step</strong> interval, then click <strong>Run</strong> to generate the score distribution.' },
  { title: 'Amount Filter',  body: 'Use the <strong>Amount Filter</strong> to narrow transactions by a numeric column. Set a start and end range, then click <strong>Run Analysis</strong> to apply it.' },
  { title: 'Reading Charts', body: 'Toggle <strong>Total</strong> and <strong>Fraud</strong> lines on each chart. Switch between <strong>bar</strong> and <strong>line</strong> styles. Click any bar to drill into that score bucket.' },
  { title: 'Decision Mode',  body: 'If <strong>Decision Mode</strong> is configured in Set Parameters, it appears in the nav. Tag buckets as <strong>Successful</strong> or <strong>Unsuccessful</strong> to filter charts by outcome.' },
];
let _SA_tourStep    = 0;
let _SA_tourEnabled = false;

function SA_tourShow(step) {
  _SA_tourStep = step;
  const s = _SA_TOUR_STEPS[step];
  if (!s) { SA_tourDismiss(); return; }
  const total = _SA_TOUR_STEPS.length;
  let el = document.getElementById('SA_TourCard');
  if (!el) {
  ...
  el.innerHTML = `
    <div class="gs-toast" id="SA_TourInner" style="border-left-color:var(--brand-sa);">
      <div class="gs-tab-strip" onclick="SA_tourTabExpand()" title="Expand">
      ...
      <div class="gs-toast-inner">
        <div class="gs-toast-header">
          <div>
            <div class="gs-toast-step" style="color:var(--brand-sa);">Step ${step+1} of ${total}</div>
            <div class="gs-toast-title">${s.title}</div>
          </div>
          <button class="gs-toast-btn-collapse" onclick="SA_tourTabCollapse()" title="Collapse to tab" style="background:var(--brand-sa-dim);border-color:var(--brand-sa);color:var(--brand-sa);">
        ...
        <div class="gs-toast-actions">
          <button class="gs-toast-btn-back" onclick="SA_tourShow(_SA_tourStep - 1)" style="${step === 0 ? 'visibility:hidden;' : ''}">← Back</button>
          <button class="gs-toast-btn-next" onclick="SA_tourNext()" style="background:var(--brand-sa);">${step+1 < total ? 'Next →' : 'Finish ✓'}</button>
        </div>
      </div>
    </div>`;
}

function SA_tourTabCollapse() {
  const el = document.getElementById('SA_TourInner');
  if (!el) return;
...
function SA_tourNext() { SA_tourShow(_SA_tourStep + 1); }

function SA_tourDismiss() {
  document.getElementById('SA_TourCard')?.remove();
  _SA_tourEnabled = false;
  document.getElementById('SA_HelpBtn')?.classList.remove('tutorial-active');
}

function SA_HelpPrompt() {
  _SA_tourEnabled = !_SA_tourEnabled;
  document.getElementById('SA_HelpBtn')?.classList.toggle('tutorial-active', _SA_tourEnabled);
  if (_SA_tourEnabled) {
    SA_tourShow(0);
  } else {
    SA_tourDismiss();
...
if (typeof App_RegisterTutorial === 'function') App_RegisterTutorial('sa-active', SA_HelpPrompt);
...
function SA_RenderGraph(key, canvasId, bins) {
  ...
  const { labels, totalVals, fraudVals, totalRaw, fraudRaw } = SA_getBinsData(bins);
    ...
    options: {
        ...
        tooltip: {
          ...
          callbacks: {
            ...
            label: item => {
              ...
              const val    = item.raw?.y ?? item.raw;
              const rawArr = name === 'Total' ? totalRaw : fraudRaw;
              const raw    = rawArr?.[idx] ?? val;
              const isCalc = _SA_Calc !== 'Count';
              const isPct  = _SA_Calc === 'Distribution' || _SA_Calc === 'Cumulative' || _SA_Calc === 'FraudRate';
              if (isCalc) {
                const pct = isPct ? `${val.toFixed(1)}%` : fmtN(val);
                return `  ${name}: ${pct}  (${fmtN(raw)})`;
              }
              return `  ${name}: ${fmtN(val)}`;
            },
            afterBody: items => {
              const idx = items[0]?.dataIndex;
              const t   = totalRaw?.[idx] ?? 0;
              const f   = fraudRaw?.[idx]  ?? 0;
              if (_SA_Calc === 'Count' && t > 0) return [`  Rate: ${(f / t * 100).toFixed(1)}%`];
              return [];
            },
          },
        },
      ...
      scales: {
        ...
        y: {
          ticks: {
            font: { size: 9 },
            callback: v => ['Distribution','Cumulative','FraudRate'].includes(_SA_Calc) ? v + '%' : fmtN(v),
          },
          beginAtZero: true,
        },
      },
