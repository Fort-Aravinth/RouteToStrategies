...
// ── Tutorial ──────────────────────────────────────────────────────────────────
const _SC_TOUR_STEPS = [
  { title: 'Score Columns',  body: 'Add up to <strong>4 score columns</strong> to compare side-by-side. Set a <strong>Start / End</strong> range for each. Click <strong>Run</strong> for raw scores or <strong>Normalised</strong> to align all scores on a 0–100% axis.' },
  { title: 'Amount Filter',  body: 'Use the <strong>Amount Filter</strong> to narrow transactions by a numeric column. Set a start and end range, then toggle the chip on any chart to apply it.' },
  { title: 'Reading Charts', body: 'Toggle <strong>Total</strong> and <strong>Fraud</strong> series — Fraud is always shown in red across all scores. Switch between <strong>bar</strong> and <strong>line</strong> styles. Use the <strong>Filters</strong> card to add post-query bin conditions.' },
  { title: 'Decision Mode',  body: 'If <strong>Decision Mode</strong> is configured in Set Parameters, it appears in the nav. Filter charts by outcome — <strong>Successful</strong> or <strong>Unsuccessful</strong> — across all compared scores at once.' },
];
let _SC_tourStep    = 0;
let _SC_tourEnabled = false;

function SC_tourShow(step) {
  _SC_tourStep = step;
  const s = _SC_TOUR_STEPS[step];
  if (!s) { SC_tourDismiss(); return; }
  const total = _SC_TOUR_STEPS.length;
  let el = document.getElementById('SC_TourCard');
  if (!el) {
  ...
  el.innerHTML = `
    <div class="gs-toast" id="SC_TourInner" style="border-left-color:var(--brand-sc);">
      <div class="gs-tab-strip" onclick="SC_tourTabExpand()" title="Expand">
      ...
      <div class="gs-toast-inner">
        <div class="gs-toast-header">
          <div>
            <div class="gs-toast-step" style="color:var(--brand-sc);">Step ${step+1} of ${total}</div>
            <div class="gs-toast-title">${s.title}</div>
          </div>
          <button class="gs-toast-btn-collapse" onclick="SC_tourTabCollapse()" title="Collapse to tab" style="background:var(--brand-sc-dim);border-color:var(--brand-sc);color:var(--brand-sc);">
        ...
        <div class="gs-toast-actions">
          <button class="gs-toast-btn-back" onclick="SC_tourShow(_SC_tourStep - 1)" style="${step === 0 ? 'visibility:hidden;' : ''}">← Back</button>
          <button class="gs-toast-btn-next" onclick="SC_tourNext()" style="background:var(--brand-sc);">${step+1 < total ? 'Next →' : 'Finish ✓'}</button>
        </div>
      </div>
    </div>`;
}

function SC_tourTabCollapse() {
  const el = document.getElementById('SC_TourInner');
  if (!el) return;
...
function SC_tourNext() { SC_tourShow(_SC_tourStep + 1); }

function SC_tourDismiss() {
  document.getElementById('SC_TourCard')?.remove();
  _SC_tourEnabled = false;
  document.getElementById('SC_HelpBtn')?.classList.remove('tutorial-active');
}

function SC_HelpPrompt() {
  _SC_tourEnabled = !_SC_tourEnabled;
  document.getElementById('SC_HelpBtn')?.classList.toggle('tutorial-active', _SC_tourEnabled);
  if (_SC_tourEnabled) {
    SC_tourShow(0);
  } else {
    SC_tourDismiss();
...
if (typeof App_RegisterTutorial === 'function') App_RegisterTutorial('sc-active', SC_HelpPrompt);
  ...
  scores.forEach(s => {
    ...
    if (_SC_RunBy.fraud) {
      datasets.push({
        ...
        backgroundColor: isArea ? 'rgba(239,68,68,0.2)' : 'rgba(239,68,68,0.8)',
...
function SC_toggleCS(id) {
  ...
  document.querySelectorAll('#SCView .custom-select.open, #SC_MiniNav .custom-select.open').forEach(s => s.classList.remove('open'));
  if (!isOpen) {
...
document.addEventListener('click', e => {
  if (!e.target.closest('#SCView .custom-select') && !e.target.closest('#SC_MiniNav .custom-select')) {
    document.querySelectorAll('#SCView .custom-select.open, #SC_MiniNav .custom-select.open').forEach(s => s.classList.remove('open'));
  }
});
