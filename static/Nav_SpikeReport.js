...
// ── Badge ─────────────────────────────────────────────────────────────────────
function SR_showBadge(type, msg) {
  let el = document.getElementById('SR_ReadyBadge');
  if (!el) { el = document.createElement('div'); el.id = 'SR_ReadyBadge'; el.className = 'App_badge'; document.body.appendChild(el); }
  if (type === 'loading') {
    el.style.borderRightColor = '';
    el.innerHTML = `<svg viewBox="0 0 16 16" width="14" height="14" style="flex-shrink:0;animation:GS_spin 1s linear infinite;color:var(--brand-dm)"><circle cx="8" cy="8" r="5.5" stroke="currentColor" stroke-width="1.5" stroke-dasharray="22" stroke-dashoffset="8" fill="none"/></svg><span>${msg}</span>`;
  } else {
    el.style.borderRightColor = 'var(--brand-dm)';
    el.innerHTML = `<svg viewBox="0 0 16 16" width="14" height="14" fill="none" style="flex-shrink:0;color:var(--brand-dm)"><circle cx="8" cy="8" r="6.5" stroke="currentColor" stroke-width="1.5"/><path d="M5 8l2 2 4-4" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg><span>${msg}</span><button onclick="document.getElementById('SR_ReadyBadge').remove();SP_stackBadges();">✕</button>`;
  }
...
// ── SR Info Popup ─────────────────────────────────────────────────────────────
function SR_infoOpen(btn, title, text) {
  const popup = document.getElementById('SR_InfoPopup');
  if (!popup) return;
  document.getElementById('SR_InfoTitle').textContent = title;
  document.getElementById('SR_InfoBody').innerHTML    = text;
  ...
  setTimeout(() => document.addEventListener('click', _SR_infoOutside), 0);
  window.addEventListener('scroll', SR_infoClose, { once: true, capture: true });
}
function _SR_infoOutside(e) {
  const popup = document.getElementById('SR_InfoPopup');
  if (popup && !popup.contains(e.target) && !e.target.classList.contains('MN_info_btn'))
    SR_infoClose();
}
function SR_infoClose() {
  const popup = document.getElementById('SR_InfoPopup');
  if (popup) popup.style.display = 'none';
  document.removeEventListener('click', _SR_infoOutside);
}
